...
mod tests {
    ...
    fn sync_loss_replication() {
        ...
        let mut corrupt = TimeSeries {
            ...
            stats: TimeSeriesStats::default(),
            first_idx: 0,
            prev_snapshot: Vec::with_capacity(300),
            upsert_type: UpsertType::default(),
            prev_value: (0, None),
        };
        let input = (65624, Some(9.0));
        let last_idx = corrupt.get_last_idx();
        assert_eq!(last_idx, 5);
        assert_eq!(input.0 as i64 - corrupt.metrics[last_idx].0 as i64,-295);
        corrupt.upsert(input);
        assert_eq!(corrupt.sanity_check(), false);
    }
}
